...
`libini.so` and `libwiringx.so` are installed to the Debian multiarch library
directory, such as `/usr/lib/riscv64-linux-gnu`; the other system libraries are
installed to `/mnt/system/lib`.
